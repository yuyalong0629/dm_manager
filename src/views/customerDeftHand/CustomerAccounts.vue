  ...
  <div class="customer-accounts">
    <a-spin :spinning="spinning">
            ...
            <a-row type="flex" justify="space-between">
              ...
              <a-col :span="6">
                <a-form-model-item label="快手号属性" prop="accountType">
                  <a-select style="width: 160px" v-model="form.accountType" placeholder="请选择快手号属性">
      ...
      <a-table
        ...
        :scroll="{ x: 2400, y: 500 }"
      >
        <span slot="isLink" slot-scope="text">{{ text === 0 ? '否' : '是' }}</span>
        <span slot="isQuickorder" slot-scope="text">{{ text === 0 ? '否' : '是'}}</span>
        ...
        <span slot="accountState" slot-scope="text">{{ text === 7 ? '快手号' : '' }}</span>
      </a-table>
      <br />
      <a-pagination
        showQuickJumper
  ...
  private handleImport() {
    if (this.selectedRowKeys) {
      accountExcel({
        ...
        link.download = `快手导出 + ${new Date().getTime()}`
        link.click()
      })
    }
  }
